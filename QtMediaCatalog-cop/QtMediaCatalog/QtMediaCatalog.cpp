	...
	: QMainWindow(parent)
{
	...
	fileTable = new QTableView(mainWdgt);
	model = Q_NULLPTR;
...
void QtMediaCatalog::SearchClicked() {
	...
	emit pathEntered("E:/Video", editExt->text());
}
void QtMediaCatalog::UpdateTable()
{
	if (model != Q_NULLPTR) { delete model; } 
	...
	connect(model, SIGNAL(fileChoosen(bool)), butAddToDir, SLOT(setEnabled(bool)));
